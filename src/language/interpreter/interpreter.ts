...
//     Backward, Forward, Loop, Robot, INT_const, Print, RobotVisitor, SpeedCommand, TurnLeft, TurnRight, CallVariable,
//     VariableDeclaration, UpdateVariable, acceptNode, And, Or, Equality, PlusMinus,INT_neg_const, Rotate, DeclaredParameter, StrafeLeft, Return, STRING_const, Atomic } from "../visitor.js";
...
//         let angle: number = 90 * Math.PI / 180; // degree to radian
...
//         let angle: number = (node.angle as any) * Math.PI / 180; //Fix the evaluation of node.angle
//         this.robot.turn(angle);
//         this.sendRobotToClient({dist: 0, strafe: 0, angle: angle});
//     }
...
//         }
//         return this.variables.get(node.name.ref?.name);
//     }
